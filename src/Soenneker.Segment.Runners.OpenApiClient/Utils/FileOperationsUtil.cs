...
using Soenneker.Segment.Runners.OpenApiClient.Utils.Abstract;
...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        string openApiDocumentUrl = _configuration["Segment:ClientGenerationUrl"] ?? "https://docs.segmentapis.com";
        ...
        await _kiotaUtil.Generate(fixedFilePath, "SegmentOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();

        await BuildAndPush(gitDirectory, cancellationToken).NoSync();
    }

    private async ValueTask ExtractOpenApiFromRedoclyState(string documentationUrl, string html, string gitDirectory, string targetFilePath,
        CancellationToken cancellationToken)
    {
        Match assetMatch = Regex.Match(html, @"(?:src|href)=[""'](?<path>[^""']*redocly-state-[^""']+\.js)[""']", RegexOptions.IgnoreCase);

        if (!assetMatch.Success)
            throw new InvalidOperationException("Segment's Redocly state asset could not be found in the documentation page.");

        string stateAssetUrl = new Uri(new Uri(documentationUrl), assetMatch.Groups["path"].Value).AbsoluteUri;
        string stateFilePath = Path.Combine(gitDirectory, "redocly-state.js");

        try
        {
            string? downloadedStatePath = await _fileDownloadUtil.Download(stateAssetUrl, stateFilePath, fileExtension: ".js",
                cancellationToken: cancellationToken);
            ...
            JsonElement openApi = stateDocument.RootElement.GetProperty("definition").GetProperty("data");

            if (!openApi.TryGetProperty("openapi", out _))
                throw new InvalidOperationException("Segment's Redocly state did not contain an OpenAPI document.");

            await _fileUtil.Write(targetFilePath, openApi.GetRawText(), cancellationToken: cancellationToken);
        }
        finally
        {
